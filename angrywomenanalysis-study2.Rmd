...
The independent samples Bayesian t-test gives infinite evidence in favor of the hypothesis that targets with anger expressions are perceived as more angry than targets with not-angry (sadness or neutral) expressions (BF$_{+0} = \infty$, $\delta$ = `r printnum(median(samp.mc.anger[,"delta"]))`). Similarly, the independent samples Bayesian t-test gives infinite evidence in favor of the hypothesis that targets with not-angry expressions are perceived as more sad than targets with angry expressions (BF$_{-0} = \infty$, $\delta$ = `r printnum(median(samp.mc.sadness[,"delta"]))`). See Figure \@ref(fig:fig-manipulation) for a plot of the data. 
...
modnames <- c("Gender Stereotyping vs. Designs", "Status Signalling vs. Designs", "Culture Change/Study Savviness vs. Designs", "Unconstrained vs. Designs", "Null vs. Designs")
...
```{r tab-status, eval=FALSE}
modnames <- c("Status Signalling vs. Gender Stereotyping", "Status Signalling vs. Culture Change/Study Savviness",
              "Status Signalling vs. Unconstrained", "Status Signalling vs. Designs", "Status Signalling vs. Null")


tab <- data.frame(Comparison = modnames, BFs = myformat(bfss.status))
row.names(tab) <- NULL
...
bayes.warmth <- doBayes(dat, dat$warmth, rscale)
bayes.control <- doBayes(dat, dat$control, rscale)

dat.dvs <- dat
```

```{r mediation, cache=TRUE, eval=FALSE}
library(brms)
f1 <- bf(warmth ~ 1 + emotion)
f2 <- bf(competence ~ 1 + emotion)
f3 <- bf(status ~ 1 + warmth + competence + emotion)
m1 <- brm(f3 + f1 + f2 + set_rescor(FALSE), data = dat.dvs)

post <- posterior_samples(m1)

post <-
  post %>% 
  mutate(`a1 emotion-warmth` = b_warmth_emotionneutral,
         `a2 emotion-competence` = b_competence_emotionneutral,
         `b1 warmth-status` = b_status_warmth,
         `b2 competence-status`= b_status_competence,
         `c_prime emotion-status total direct effect`= b_status_emotionneutral) %>% 
  mutate(`a1b1 emotion-warmth-status`= `a1 emotion-warmth` * `b1 warmth-status`,
         `a2b2 emotion-competence-status`= `a2 emotion-competence` * `b2 competence-status`) %>% 
  mutate(`c total effect`    = `c_prime emotion-status total direct effect` + `a1b1 emotion-warmth-status` + `a2b2 emotion-competence-status`,
         `total indirect effect` = `a1b1 emotion-warmth-status` + `a2b2 emotion-competence-status`) 
...
post %>% 
  pivot_longer(cols = `a1 emotion-warmth`:`total indirect effect`) %>%
  group_by(name) %>% 
  median_qi(value) %>% 
  mutate_if(is.double, round, digits = 3)
```

```{r fig-main, eval = T, fig.cap="Descriptive plots of **A.** status conferral, **B.** competence, **C.** warmth, and **D.** Out of control per target gender and emotion.", fig.width=8, fig.asp=.8}
...
```{r table-all}
bfs0.competence <- c(bayes.competence$bfsb[1:5],0)
bfs0.warmth <- bayes.warmth$bfsb
bfs0.control <- c(bayes.control$bfsb[1:5],0)
...
apa_table(data.frame(Comparison = names(bfs0.warmth), 
                     Status = c(makeLargestBold(bayes.status$bfsb)[1:5],"--"), 
                     Competence = c(makeLargestBold(bfs0.competence)[1:5],"--"), 
                     Warmth = makeLargestBold(bfs0.warmth), 
                     `Out of control` = c(makeLargestBold(bfs0.control)[1:5],"--")),
          col_spanners = list(`Bayes Factors` = c(2,5)),
          col.names = c("Comparison", "Status","Competence","Warmth","Out of Control"),
          row.names = FALSE,
          align = c("l","c","c","c","c"),
          caption = "Evidence for the theoretical models vs. the baseline (intercepts only) models for the different dependent variables. ", 
          escape=F,
...
bayes.re.comp <- doRandom(dat.dvs, dat.dvs$competence, rscale)
bayes.re.warm <- doRandom(dat.dvs, dat.dvs$warmth, rscale)
bayes.re.cont <- doRandom(dat.dvs, dat.dvs$control, rscale)
```
...
As visualized in Figure \@ref(fig:plotter), however, there is no indication that the target emotion effect and the gender-by-emotion interaction effect vary substantially of systematically across designs. Rather, the 95% credible intervals includes zero across almost all designs for the main effect of target emotion, as well as the interaction effect. For the target emotion effect, angry targets are accorded more status than non-angry targets across `r length(n.angry.stat)` designs, while the opposite is true for `r length(n.nonangry.stat)` design. For the target gender effect, there are `r length(des.male)+length(des.female)` designs for which the coefficient of the target gender effect does not include 0. Across those designs, male targets are accorded higher status than female targets (regardless of emotion condition) for `r length(des.male)` designs and females are accorded higher status than males for `r length(des.female)` design. See Figure \@ref(fig:plotter) which designs show these effects. Corroborating the visual pattern, the Bayes factor analysis suggests that the unconstrained random effects model strongly outperforms the baseline-model that includes only varying intercepts for site and design; BF$_{rb}=$ `r myformat(1/bayes.re$BF_br)`. Note however, that the pattern of between design variation in the size and direction of the target gender effect is not predicted by any of the theoretical models. 
...
Figure \@ref(fig:plotter-comp) shows the intercepts and varying effects across design for competence ratings as the outcome measure. In line with the Bayes factor analysis providing most evidence for the unconstrained model, the pattern in panel C displays a *negative* effect of anger vs. non-anger across `r length(n.nonangry.comp)` designs (i.e., an effect opposite to predictions by the Status Signalling perspective). None of the designs show a target gender effect or a gender-by-emotion interaction effect. As predicted by the 'Anger suppresses warmth' perspective, a similar pattern emerges for perceived warmth in Figure \@ref(fig:plotter-warmth): across `r length(n.nonangry.warm)` designs the 95% credible interval of the coefficient for target emotion excludes zero, with non-angry targets considered warmer than angry targets. In addition, across `r length(n.female.warm)` designs, female targets are considered warmer than male targets, while for `r length(n.male.warm)` designs, male targets are considered warmer than female targets. Finally, in `r length(n.stereo.warm)` designs, a gender stereotype effect is observed such that an angry male target is considered warmer than a non-angry male targets and an angry female target, while an angry female target is considered less warm than a non-angry female target and an angry male target. 
For out-of-control ratings, a reverse pattern for the target emotion effect is shown in Figure \@ref(fig:plotter-control): across `r length(n.angry.cont)` designs the 95% credible interval of the coefficient for target emotion excludes zero, with angry targets considered more out of control than non-angry targets. 

```{r plotter, fig.width=6.5, fig.asp=1.2, fig.cap= "Estimated design-level effects (posterior means) on status conferral, in increasing order. \\textbf{A.} Intercepts. \\textbf{B.} Target gender effects. \\textbf{C.} Target emotion effects. \\textbf{D.} Gender-by-emotion interaction effects. Each dot represents a design. The horizontal lines denote the 95\\% credible intervals. Estimated effects for which the 95\\% credible interval includes zero are displayed in grey and estimated effects for which the 95\\% credible intervals exclude zero are displayed in red. Only the target gender effect seem to differ from zero for some designs."}
par(mfrow=c(2,2))
...
```{r plotter-comp, fig.width=6.5, fig.asp=1.2, fig.cap= "Estimated design-level effects (posterior means) on competence, in increasing order. \\textbf{A.} Intercepts. \\textbf{B.} Target gender effects. \\textbf{C.} Target emotion effects. \\textbf{D.} Gender-by-emotion interaction effects. Each dot represents a design. The horizontal lines denote the 95\\% credible intervals. Estimated effects for which the 95\\% credible interval includes zero are displayed in grey and estimated effects for which the 95\\% credible intervals exclude zero are displayed in red. A target emotion effect emerges across about half of the designs, yet in the direction opposite as predicted by the Status Signalling perspective."}
par(mfrow=c(2,2))
J <- length(unique(designs))
colors <- rep("grey36", J) #hsv((1:J)/J,1,.7,1)
...
```{r plotter-warmth, fig.width=6.5, fig.asp=1.2, fig.cap= "Estimated design-level effects (posterior means) on warmth, in increasing order. \\textbf{A.} Intercepts. \\textbf{B.} Target gender effects. \\textbf{C.} Target emotion effects. \\textbf{D.} Gender-by-emotion interaction effects. Each dot represents a design. The horizontal lines denote the 95\\% credible intervals. Estimated effects for which the 95\\% credible interval includes zero are displayed in grey and estimated effects for which the 95\\% credible intervals exclude zero are displayed in red. In line with the Anger Suppresses Warmth perspective, a target emotion effect emerges across most designs."}
par(mfrow=c(2,2))
J <- length(unique(designs))
colors <- rep("grey36", J) #hsv((1:J)/J,1,.7,1)
...
```{r plotter-control, fig.width=6.5, fig.asp=1.2, fig.cap= "Estimated design-level effects (posterior means) on out-of-control ratings, in increasing order. \\textbf{A.} Intercepts. \\textbf{B.} Target gender effects. \\textbf{C.} Target emotion effects. \\textbf{D.} Gender-by-emotion interaction effects. Each dot represents a design. The horizontal lines denote the 95\\% credible intervals. Estimated effects for which the 95\\% credible interval includes zero are displayed in grey and estimated effects for which the 95\\% credible intervals exclude zero are displayed in red. In line with the Status Signalling perspective, a target emotion effect emerges across about half of the designs."}
par(mfrow=c(2,2))
J <- length(unique(designs))
colors <- rep("grey36", J) #hsv((1:J)/J,1,.7,1)
...
```{r plotter-combined, fig.width=6.5, fig.asp=1.4, fig.cap="Estimated design-level effects (posterior means), in increasing order. \\textbf{A.} Target gender effect on status conferral. \\textbf{B.} Target emotion effects on status conferral. \\textbf{C.} Target gender-by-emotion interaction effect on status conferral. \\textbf{D.} Target emotion effects on warmth. \\textbf{E.} Target emotion effect on competence. \\textbf{F.} Target emotion effect on out-of-control ratings. Each dot represents a design. The horizontal lines denote the 95\\% credible intervals. Estimated effects for which the 95\\% credible interval includes zero are displayed in grey and estimated effects for which the 95\\% credible intervals exclude zero are displayed in red."}
par(mfrow=c(3,2))
...
```{r plot-design-moderators, fig.width=6.5, fig.asp=1.4, fig.cap="Varying effect of target gender per design, colored based on design-related moderators."}
...
plotter(bayes.re$betas, c(-1.5,1.5), designs, cols.5, main = "E. Type of Design", labs = c("Female","Male"), leg = c("adapted","novel","original"))
...
For status conferral as the outcome variable, we further investigated to what extent the target gender effect might be explained by design-related moderators. 
...
bf.anger.extr  <- as.vector(ttestBF(formula = status ~ extremity, data = dat_sub, nullInterval = c(-Inf,0)))[1]
...
__*Anger Extremity Hypothesis A.*__ We predicted that moderate levels of anger would lead to more status conferral than extreme anger for both targets. To this end, we compared status conferral to angry targets between designs that were identical except for the extremity of the displayed anger. 
However, the data indicated that mild anger led to *less* status conferral compared to extreme anger ($M_{mild}=$ `r round(mean(dat_sub$status[dat_sub$extremity=="mild"]),2)`); $M_{extreme}=$ `r round(mean(dat_sub$status[dat_sub$extremity=="extreme"]),2)`). As the effect went in the opposite direction, the Bayes factor analysis also provided evidence against the anger extremity hypothesis: BF$_{10}=$ `r myformat(bf.anger.extr)`; BF$_{01}=$ `r myformat(1/bf.anger.extr)`. 
...
```{r main-gender}
bf.targ.gender <- as.vector(ttestBF(formula = status ~ gender, data = dat.dvs))[1]
bf.targ.gender.comp <- as.vector(ttestBF(formula = competence ~ gender, data = dat.dvs))[1]
bf.targ.gender.warm <- as.vector(ttestBF(formula = warmth ~ gender, data = dat.dvs))[1]
bf.targ.gender.cont <- as.vector(ttestBF(formula = control ~ gender, data = dat.dvs))[1]
```

__*Main effect of target.*__ Are male or female targets generally accorded more status? 
We ran a simple independent-samples t-test to answer this question. In contrast to Study 1, female targets are accorded slightly *less* status than male targets ($M_{female}=$ `r round(mean(dat$status[dat$gender=="female"]),2)`; $M_{male}=$ `r round(mean(dat$status[dat$gender=="male"]),2)`), yet the Bayes factor analysis provides moderate evidence against the gender effect: BF$_{10}=$ `r myformat(bf.targ.gender)`; BF$_{01}=$ `r myformat(1/bf.targ.gender)` (Frequentist statistics: `r apa_print(t.test(status~gender, data=dat))$statistic`). For the other dependent variables, we also found evidence in favor of the null-hypotheses; for competence BF$_{10}=$ `r myformat(bf.targ.gender.comp)`; BF$_{01}=$ `r myformat(1/bf.targ.gender.comp)`, for warmth BF$_{10}=$ `r myformat(bf.targ.gender.warm)`; BF$_{01}=$ `r myformat(1/bf.targ.gender.warm)`, and for out-of-control BF$_{10}=$ `r myformat(bf.targ.gender.cont)`; BF$_{01}=$ `r myformat(1/bf.targ.gender.cont)`. 

```{r main-emotion}
bf.targ.emo <- as.vector(ttestBF(formula = status ~ emotion, data = dat.dvs))[1]
bf.targ.emo.comp <- as.vector(ttestBF(formula = competence ~ emotion, data = dat.dvs))[1]
bf.targ.emo.warm <- as.vector(ttestBF(formula = warmth ~ emotion, data = dat.dvs))[1]
bf.targ.emo.cont <- as.vector(ttestBF(formula = control ~ emotion, data = dat.dvs))[1]
```

__*Main effect of emotion.*__ Are angry or not-angry targets generally accorded more status? 
A simple independent-samples t-test showed evidence against a main effect of emotion on status conferral: BF$_{10}=$ `r myformat(bf.targ.emo)`; BF$_{01}=$ `r myformat(1/bf.targ.emo)` (Frequentist statistics: `r apa_print(t.test(status~emotion, data=dat))$statistic`). For the other dependent variables, we did find evidence in favor of a main effect of emotion; for competence BF$_{10}=$ `r myformat(bf.targ.emo.comp)`, for warmth BF$_{10}=$ `r myformat(bf.targ.emo.warm)`, and for out-of-control BF$_{10}=$ `r myformat(bf.targ.emo.cont)`. As becomes evident from Figure \@ref(fig:fig-main), for competence and warmth, angry targets are considered less competent and less warm than non-angry targets, respectively, and for out of control, angry targets are considered more out of control than non-angry targets.  
...
do.multiverse = function(dat, rscale, bayes.main){
  ...
  d <- dat[dat$pass.1==1,]
  subs[1] <- nrow(d)
  bayes.multiverse[[1]] <- doBayes(d, d$status, rscale)
  d <- dat[dat$pass.2==1,]
  subs[2] <- nrow(d)
  bayes.multiverse[[2]] <- doBayes(d, d$status, rscale)
  d <- dat[dat$pass.4==1,]
  subs[3] <- nrow(d)
  bayes.multiverse[[3]] <- doBayes(d, d$status, rscale)
  d <- dat[dat$pass.5==1,]
  subs[4] <- nrow(d)
  bayes.multiverse[[4]] <- doBayes(d, d$status, rscale)
  d <- dat[dat$pass.1==1&dat$pass.2==1,]
  subs[5] <- nrow(d)
  bayes.multiverse[[5]] <- doBayes(d, d$status, rscale)
  d <- dat[dat$pass.1==1&dat$pass.4==1,]
  subs[6] <- nrow(d)
  bayes.multiverse[[6]] <- doBayes(d, d$status, rscale)
  d <- dat[dat$pass.1==1&dat$pass.5==1,]
  subs[7] <- nrow(d)
  bayes.multiverse[[7]] <- doBayes(d, d$status, rscale)
  d <- dat[dat$pass.2==1&dat$pass.4==1,]
  subs[8] <- nrow(d)
  bayes.multiverse[[8]] <- doBayes(d, d$status, rscale)
  d <- dat[dat$pass.2==1&dat$pass.5==1,]
  subs[9] <- nrow(d)
  bayes.multiverse[[9]] <- doBayes(d, d$status, rscale)
  d <- dat[dat$pass.4==1&dat$pass.5==1,]
  subs[10] <- nrow(d)
  bayes.multiverse[[10]] <- doBayes(d, d$status, rscale)
  d <- dat[dat$pass.1==1&dat$pass.2==1&dat$pass.4==1,]
  subs[11] <- nrow(d)
  bayes.multiverse[[11]] <- doBayes(d, d$status, rscale)
  d <- dat[dat$pass.1==1&dat$pass.2==1&dat$pass.5==1,]
  subs[12] <- nrow(d)
  bayes.multiverse[[12]] <- doBayes(d, d$status, rscale)
  d <- dat[dat$pass.1==1&dat$pass.4==1&dat$pass.5==1,]
  subs[13] <- nrow(d)
  bayes.multiverse[[13]] <- doBayes(d, d$status, rscale)
  d <- dat[dat$pass.2==1&dat$pass.4==1&dat$pass.5==1,]
  subs[14] <- nrow(d)
  bayes.multiverse[[14]] <- doBayes(d, d$status, rscale)
  d <- dat[dat$pass.1==1&dat$pass.2==1&dat$pass.4==1&dat$pass.5==1,]
  subs[15] <- nrow(d)
  bayes.multiverse[[15]] <- doBayes(d, d$status, rscale)
...
In the preregistration we specified that we would only analyze a specific exclusion-based multiverse path when the exclusion affect at least 5% of the sample. However, the straightlining criterion did not reach this threshold; only `r sum(dat$pass.3==0, na.rm=T)/nrow(dat)*100`% of the sample was excluded because of straightlining across all 5 scales. The language fluency (`r sum(dat$pass.1==0, na.rm=T)/nrow(dat)*100`%), target gender manipulation check item (`r sum(dat$pass.2==0, na.rm=T)/nrow(dat)*100`%), the material quality (`r sum(dat$pass.4==0, na.rm=T)/nrow(dat)*100`%), and the mode of administration (`r sum(dat$pass.5==0, na.rm=T)/nrow(dat)*100`%) did result in a meaningful proportion of exclusion. There was only 1 design (i.e., design 7) for which there was no compelling evidence that targets in the anger condition were perceived as angrier than in the not-angry condition (BF$_{10}=$ `r BF_emo[BF_emo<10]`). This, however, meant (`r sum(dat$pass.6b==0)/nrow(dat)*100`%) of the sample, which does not pass the threshold of 5%. 
...
yrange <- c(pretty(range(dd$BFs.1))[1], length(pretty(range(dd$BFs.1)))+1)
...
plot(dd$n, dd$BFs.1, 
     ylim = yrange,
     xlim = pretty(range(subs))[c(1,length(pretty(range(subs))))], 
     axes=F, xlab="Sample Size", ylab="", type = "n")
abline(h=1, lty=1)
points(dd$n,dd$BFs.1
       , pch=19
       , col=alpha(cols[dd$id],0.6)
       ,cex=1.2)
axis(1,at=pretty(range(subs)))
axis(2)
title(ylab = expression(BF["theory vs. baseline"]))    
legend("topright", legend = c("Gender Stereotyping", "Status Signalling", "Culture Change/Study Savviness"), 
         col = alpha(cols[dd$id],0.6), pch = 19, bty = "n")
...
modnames.competence <- c("Gender Stereotyping vs. Designs", "Status Signalling vs. Designs", "Culture Change/Study Savviness vs. Designs", "Unconstrained vs. Designs", "Null vs. Designs")
...
modnames.warmth <- c("Gender Stereotyping vs. Designs", "Status Signalling vs. Designs", "Culture Change/Study Savviness vs. Designs", "Unconstrained vs. Designs", "Null vs. Designs","Warmth vs. Designs")
...
modnames.control <- c("Gender Stereotyping vs. Designs", "Status Signalling vs. Designs", "Culture Change/Study Savviness vs. Designs", "Unconstrained vs. Designs", "Null vs. Designs")
...
bayes.original.gender <- as.vector(ttestBF(formula = status ~ gender, data = original))
```

__*Target Gender Effect for the Brescoll and Uhlmann (2008) design across Studies 1 and 2.*__
Our present Study 1 finds a tendency for greater to status attribution to women, whereas Study 2 finds greater status attribution to men. If we only consider the original design (i.e., design 27) in Study 2, we find no evidence for a target gender effect: BF$_{10}=$ `r bayes.original.gender`; BF$_{01}=$ `r 1/bayes.original.gender`, $M_{male}=$ `r mean(original$status[original$gender=="male"])`, $M_{female}=$ `r mean(original$status[original$gender=="female"])`. The absence of a gender difference in the original design can also be observed from Figure \@ref(fig:plot-design-moderators)E, where the target gender coefficient is almost exactly 0. 

```{r gaertig_replication}
gaertig <- subset(dat, design==7|design==8)
bayes.gaertig <- as.vector(ttestBF(formula = status ~ design, data = gaertig))
bayes.gaertig.dir <- as.vector(ttestBF(formula = status ~ design, data = gaertig, nullInterval = c(0,Inf)))[1]
```
...
pmain <- ggplot(gaertig, aes(x = perc.anger, y = jitter(status), color = condition), size = 2.2)+
  ...
  ylab("Status") +
  guides(color=guide_legend(title="Emotion | Design"))
...
__*Anger Extremity Effect for the Gaertig et al. (2019) design.*__
We conducted a directed Bayesian t-test to see if we can replicate Gaertig et al.'s finding that moderate anger leads to more status conferral than extreme anger (regardless of target gender), by looking at their design (i.e., 7&8) in isolation. However, the data indicate *less* status conferral to moderate anger ($M_{moderate}=$ `r mean(gaertig$status[gaertig$design=="7"])`) than to extreme anger ($M_{extreme}=$ `r mean(gaertig$status[gaertig$design=="8"])`). The directed Bayesian t-test confirms strong evidence against the hypothesis: BF$_{+0}=$ `r bayes.gaertig.dir`; BF$_{0+}=$ `r 1/bayes.gaertig.dir`. In fact, when we remove the directionality constraint, we get some moderate evidence for a difference (in the opposite direction): BF$_{10}=$ `r bayes.gaertig`. 

Relatedly, we explored how the Gaertig et al. (2019) design compared to the others that directly manipulated anger extremity. To unpack the anger extremity effect on status conferral, we estimated the overall status conferral per design, using varying intercepts per design and a fixed effect of the extremity manipulation. Figure \@ref(fig:plot-extremity) shows the estimated status conferral for the 18 designs in which extremity was manipulated directly, as well as the perceived anger extremity as rated by participants in the anger condition. Notably, the extremity effect (i.e., difference between the mild and extreme version) is the largest in the original Gaertig et al. (2019) design, yet in the opposite direction as found by Gaertig et al. (2019) Furthermore, while perceived anger extremity is consistently higher in the the extreme anger version of each design, the difference is clearly largest in the original Gaertig et al. design (7&8). 

```{r plot-extremity, fig.cap="Estimated status conferral per design, colored based on anger extremity. Estimates are grouped per design and ordered based on the grouped average. The x's reflect the average perceived anger extremity in the anger condition per design, as rated by participants. The x-axis at the top gives the scale of the perceived anger extremity ratings (on a 0-100 scale) and the x-axis at the bottem gives scale of the estimated status conferral (on a 1-10 scale). "}
...
bayes.status.single <- doBayes(dat.single, dat.single$status, rscale)
...
dd$Comparison <- factor(dd$Comparison, levels = c("Gender Stereotyping vs. Designs",
                                                  "Status Signalling vs. Designs",
                                                  "Culture Change/Study Savviness vs. Designs"))
...
Cell means are calculated based using the estimated parameters; for the not-angry women condition, for instance, this results in the following: $Y_{\cdot \cdot 1 1}  = -1/2 \beta  -1/2 \gamma + 1/2 \theta.$
...
where parameter $\delta$ is the effect of emotion for female targets, parameter $\eta$ is the effect of angry women versus men (both not-angry and angry). The indicator variables are $x_{4lm}$ (-1/2 for not-angry female targets and 1/2 for angry female targets, and 0 for men) and $x_{5lm}$ (-1/3 for men, 2/3 for angry women and 0 for not-angry women). 
