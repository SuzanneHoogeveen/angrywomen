...
status <- rowMeans(data[,c('Status.1','Power','Independence')])
...
temp <- na.omit(data[,c('Power','Independence','Status.1')])
iic1 <- mean(c(cor(temp[,1],temp[,2]),cor(temp[,1],temp[,3]),cor(temp[,2],temp[,3])))
temp <- na.omit(data[,c('Competent_R','Knowledgeable_R')])
iic2 <- cor(temp[,1],temp[,2])
temp <- na.omit(data[,c('Likeable','Warm')])
...
plotter(bayes.re$alphas, c(4,7),  countries, colors, main = "A. Overall Status Conferral", labs = c("Low","High"), noylab = TRUE)
plotter(bayes.re$betas,  c(-1,1), countries, colors, main = "B. Target Gender Effect", labs = c("Female","Male"), noylab = TRUE)
plotter(bayes.re$gammas, c(-1,1), countries, colors, main = "C. Target Emotion Effect", labs = c("Sad","Angry"), noylab = TRUE)
plotter(bayes.re$thetas, c(-1,1), countries, colors, main = "D. Gender-by-Emotion Effect", labs = c("Reversed Stereotype","Stereotype"), noylab = TRUE)
...
We ran a simple independent-samples t-test to answer this question. Female targets are accorded slightly more status than male targets ($M_{female}=$ `r round(mean(dat$status[dat$gender=="Female"]),2)`; $M_{male}=$ `r round(mean(dat$status[dat$gender=="Male"]),2)`)), yet the Bayes factor analysis provides some slight evidence in favor of the null-hypothesis that there is no difference in status conferral between male and female targets: BF$_{10}=$ `r myformat(bf.targ.gender)`; BF$_{01}=$ `r myformat(1/bf.targ.gender)`. (Frequentist statistics: `r apa_print(t.test(status~gender, data=dat))$statistic`)
...
Figure \@ref(fig:fig-rater) might suggest a hint of a threeway rater gender by target gender by target emotion interaction, such that female raters confer more status to a sad female target while male raters confer more status to an angry female target. However, the Bayes factor analysis indicates that the data is at least equally plausible under the (simple) null-model as under the (complex) moderator interaction model; BF$_{0m}=$ `r myformat(bayes.rater$bfs0[4])`. Finally, the main effect of target gender was tested separately for male and female raters. For female participants, there was slight evidence that female targets ($M_{female}=$ `r mean(dat$status[dat$gender=="Female"&dat$rater.gender=="female"])`) were accorded more status than male targets ($M_{male}=$ `r mean(dat$status[dat$gender=="Male"&dat$rater.gender=="female"])`): BF$_{10}=$ `r bf.targ.gender.women`. For male participants, the analysis indicated strong evidence *against* a difference ($M_{female}=$ `r mean(dat$status[dat$gender=="Female"&dat$rater.gender=="male"])`; $M_{male}=$ `r mean(dat$status[dat$gender=="Male"&dat$rater.gender=="male"])`): BF$_{10}=$ `r bf.targ.gender.men`, BF$_{01}=$ `r 1/bf.targ.gender.men`. However, there was no evidence for an interaction: BF$_{10}=$ `r bf.interaction.gender`, BF$_{01}=$ `r 1/bf.interaction.gender`.

```{r other-dvs-analysis, cache=T}
# recode competence item and create composite
competence <- 12 - rowMeans(data[,c('Competent_R','Knowledgeable_R')])
likeability <- rowMeans(data[,c('Likeable','Warm')])
...
# main effect of target excluding target gender manipulation failures. 
bf.targ.gender.excl <- as.vector(ttestBF(formula = status ~ gender, data = dat))[1]
samp.targ.gender.excl <- ttestBF(formula = status ~ gender, data = dat, posterior=T, iterations=5000)
...
__*Multiverse: Main effect of target.*__ As a follow-up, we also assessed the evidence for the main effect of target gender on status conferral when excluding participants who failed to recall the target gender correctly. The Bayes factor analysis provides moderate evidence in favor of the null-hypothesis that there is no difference in status conferral between male and female targets: BF$_{10}=$ `r myformat(bf.targ.gender.excl)`; BF$_{01}=$ `r myformat(1/bf.targ.gender.excl)`. 
...
Cell means are calculated based using the estimated parameters; for the sad women condition, for instance, this results in the following: $\bar{Y}_{\cdot \cdot 1 1}  = -1/2 \beta  -1/2 \gamma + 1/2 \theta.$

To satisfy the theoretical predictions, the following inequality constraints have to hold:
\[
\begin{aligned}
\bar{Y}_{\cdot \cdot 1 1} > \bar{Y}_{\cdot \cdot 1 2} \\
\bar{Y}_{\cdot \cdot 2 2} > \bar{Y}_{\cdot \cdot 2 1}
\end{aligned}
\]
...
\[
\begin{aligned}
\bar{Y}_{\cdot \cdot 1 2} > \bar{Y}_{\cdot \cdot 1 1} \\
\bar{Y}_{\cdot \cdot 2 2} > \bar{Y}_{\cdot \cdot 2 1}
\end{aligned}
\]
...
\[
\begin{aligned}
\bar{Y}_{\cdot \cdot 1 2 1} > \bar{Y}_{\cdot \cdot 1 1 1} \\
\bar{Y}_{\cdot \cdot 2 2 1} > \bar{Y}_{\cdot \cdot 2 1 1} \\
\bar{Y}_{\cdot \cdot 1 1 2} > \bar{Y}_{\cdot \cdot 1 2 2} \\
\bar{Y}_{\cdot \cdot 2 1 2} > \bar{Y}_{\cdot \cdot 2 2 2}
\end{aligned}
\]
